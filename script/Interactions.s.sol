...
contract FundSubscription is Script, CodeConstants {
    ...
    function fundSubscription(address vrfCoordinator, uint256 subscriptionId, address linkTokenAddress) public {
        ...
        if (block.chainid == LOCAL_CHAIN_ID) {
            ...
            console.log(LinkToken(linkTokenAddress).balanceOf(msg.sender));
            console.log(msg.sender);
            console.log(LinkToken(linkTokenAddress).balanceOf(address(this)));
            console.log(address(this));
            vm.startBroadcast();
            LinkToken(linkTokenAddress).transferAndCall(vrfCoordinator, FUND_AMOUNT, abi.encode(subscriptionId));
            vm.stopBroadcast();
        }
    }
